...
def compare_files_with_difftastic(file1, file2, colors):
    try:
        result = subprocess.run(
            [
                "difft",
                f"--color={'always' if colors else 'never'}",
                "--exit-code",
                file1,
                file2,
            ],
...
def main(directory, colors):
    sync_conflict_files = list(
        filter(
            lambda conflict: "/.stversions/" not in conflict,
            find_sync_conflict_files(directory),
        )
    )
    ...
    for conflict_file in sync_conflict_files:
        ...
        if os.path.exists(original_file):
            diff_output = compare_files_with_difftastic(
                conflict_file, original_file, colors
            )
